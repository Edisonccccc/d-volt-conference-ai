...
def transcribe_audio(audio_path: str) -> str:
    ...
    Returns the transcript text. Raises on errors so the pipeline can mark
    the conversation as errored.
    """
    p = Path(audio_path)
    if not p.is_file():
    ...
    _say(f"received transcript: {len(text)} chars in {elapsed:.1f}s")
    if text:
        # First line preview so the user sees we got something useful.
        first = text.split("\n", 1)[0]
